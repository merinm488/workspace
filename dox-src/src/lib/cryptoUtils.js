/**
 * Cryptographic utilities for secure key hashing
 */

/**
 * Simple SHA-256 implementation for non-secure contexts
 * Fallback when crypto.subtle is not available
 */
async function simpleSHA256(message) {
  // A simple implementation of SHA-256 for non-secure contexts
  // This is a basic implementation that works in non-secure contexts

  function rotateRight(n, x) {
    return ((x >>> n) | (x << (32 - n))) >>> 0;
  }

  function sha256(data) {
    const K = [
      0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
      0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
      0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
      0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
      0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
      0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
      0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
      0x748f82ae, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
    ];

    const HASH = [
      0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
    ];

    const W = new Uint32Array(64);

    function preprocess(data) {
      const bytes = new Uint8Array(data);
      const len = bytes.length;

      // Append padding
      const newLen = len + 1 + 8;
      const paddedLen = Math.ceil(newLen / 64) * 64;
      const padded = new Uint8Array(paddedLen);

      padded.set(bytes);
      padded[len] = 0x80;

      // Append length in bits (big-endian)
      const view = new DataView(padded.buffer);
      view.setUint32(paddedLen - 8, len * 0x20000000, true);
      view.setUint32(paddedLen - 4, 0, true);

      return padded;
    }

    function processChunk(chunk, H) {
      for (let i = 0; i < 16; i++) {
        W[i] = chunk[i];
      }
      for (let i = 16; i < 64; i++) {
        const s0 = rotateRight(7, W[i - 15]) ^ rotateRight(18, W[i - 15]) ^ (W[i - 15] >>> 3);
        const s1 = rotateRight(17, W[i - 2]) ^ rotateRight(19, W[i - 2]) ^ (W[i - 2] >>> 10);
        W[i] = (W[i - 16] + s0 + W[i - 7] + s1) >>> 0;
      }

      let [a, b, c, d, e, f, g, h] = H;

      for (let i = 0; i < 64; i++) {
        const S1 = rotateRight(6, e) ^ rotateRight(11, e) ^ rotateRight(25, e);
        const ch = (e & f) ^ (~e & g);
        const temp1 = (h + S1 + ch + K[i] + W[i]) >>> 0;
        const S0 = rotateRight(2, a) ^ rotateRight(13, a) ^ rotateRight(22, a);
        const maj = (a & b) ^ (a & c) ^ (b & c);
        const temp2 = (S0 + maj) >>> 0;

        h = g;
        g = f;
        f = e;
        e = (d + temp1) >>> 0;
        d = c;
        c = b;
        b = a;
        a = (temp1 + temp2) >>> 0;
      }

      H[0] = (H[0] + a) >>> 0;
      H[1] = (H[1] + b) >>> 0;
      H[2] = (H[2] + c) >>> 0;
      H[3] = (H[3] + d) >>> 0;
      H[4] = (H[4] + e) >>> 0;
      H[5] = (H[5] + f) >>> 0;
      H[6] = (H[6] + g) >>> 0;
      H[7] = (H[7] + h) >>> 0;
    }

    const preprocessed = preprocess(data);
    const H = [...HASH];

    for (let i = 0; i < preprocessed.length; i += 64) {
      const chunk = new Uint32Array(preprocessed.buffer, i, 16);
      processChunk(chunk, H);
    }

    return new Uint8Array(new Uint32Array(H).buffer);
  }

  const encoder = new TextEncoder();
  const data = encoder.encode(message);
  const hashBuffer = sha256(data);
  return hashBuffer;
}

/**
 * Generate SHA-256 hash of a string
 * @param {string} input - The key or any string to hash
 * @returns {Promise<string>} - Hexadecimal hash string
 */
export async function generateHash(input) {
  const encoder = new TextEncoder();
  const data = encoder.encode(input);

  let hashBuffer;

  // Try using crypto.subtle (available in secure contexts)
  if (typeof crypto !== 'undefined' && crypto.subtle && crypto.subtle.digest) {
    try {
      hashBuffer = await crypto.subtle.digest('SHA-256', data);
    } catch (e) {
      // Fallback to simple implementation if crypto.subtle fails
      hashBuffer = await simpleSHA256(input);
    }
  } else {
    // Use fallback implementation in non-secure contexts
    hashBuffer = await simpleSHA256(input);
  }

  const hashArray = Array.from(new Uint8Array(hashBuffer));
  const hashHex = hashArray.map(byte => byte.toString(16).padStart(2, '0')).join('');
  return hashHex;
}


/**
 * Validate that a key is not empty
 * @param {string} key - The key to validate
 * @returns {boolean} - Whether the key is valid
 */
export function isValidKey(key) {
  return key && typeof key === 'string' && key.trim().length > 0;
}

/**
 * Normalize a key by trimming whitespace
 * @param {string} key - The key to normalize
 * @returns {string} - Normalized key (trimmed)
 */
export function normalizeKey(key) {
  if (!key) return '';
  return key.trim();
}
